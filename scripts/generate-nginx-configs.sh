...
# Optional: regenerate only one project (key in projects.conf)
TARGET_PROJECT="${1:-}"

if [ -n "$TARGET_PROJECT" ]; then
  project_exists=$(yq eval "has(\"$TARGET_PROJECT\")" "$CONFIG_FILE" 2>/dev/null || echo "false")
  if [ "$project_exists" != "true" ]; then
    echo "Error: Project '$TARGET_PROJECT' not found in projects.conf"
    exit 1
  fi
...
if [ -n "$TARGET_PROJECT" ]; then
  # Remove only generated configs for the selected project
  conf_file="$NGINX_CONF_D/$TARGET_PROJECT.conf"
  if [ -f "$conf_file" ] && grep -q "# Generated by generate-nginx-configs.sh" "$conf_file" 2>/dev/null; then
    rm -f "$conf_file"
  fi

  rm -f "$NGINX_STREAM_D/${TARGET_PROJECT}-mysql.conf" "$NGINX_STREAM_D/${TARGET_PROJECT}-postgres.conf" 2>/dev/null || true
else
...
if [ -n "$TARGET_PROJECT" ]; then
  echo "Generating nginx configuration for project: $TARGET_PROJECT"
else
...
for project in $projects; do
  # Extra guard (mainly for future-proofing if projects list contains more)
